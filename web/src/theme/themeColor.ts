...
export type FrontendType =
  (typeof FRONTEND_TYPE)[keyof typeof FRONTEND_TYPE];
...
export function detectFrontendType(): FrontendType {
  try {
    const settings = (window as any)?.app?.ui?.settings;

    const vueNodes =
      settings?.getSettingValue?.("Comfy.VueNodes.Enabled");

    if (vueNodes === true) return FRONTEND_TYPE.VUE;
    if (vueNodes === false) return FRONTEND_TYPE.LITEGRAPH;
  } catch {}

  if (
    (window as any)?.comfyAPI?.app?.vueApp ||
...
export function injectTheme(theme: RuntimeTheme) {
  ...
  root.style.setProperty(
    `${CSS_VAR_PREFIX}is-classic`,
    theme._isClassic ? "1" : "0"
  );
